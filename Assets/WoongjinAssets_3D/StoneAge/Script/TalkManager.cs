...
public class TalkManager : MonoBehaviour
{
    ...
    {
        talkButtonObj.gameObject.SetActive(isActive);
    }
    ...
    public void Talk(int id, bool isNPC, NPCQuestState state, int currentQuest)
    {
        ...
        {
            talkData = GetTalk(id, 0);
        }
        else if (state == NPCQuestState.HAVE_QUEST)
        {
            talkData = GetTalk(id + (int)state + (10 * (currentQuest + 1)), talkIndex); // 1000 + 0 + 10 = 1010
        }
